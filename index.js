...
io.on("connection", function(socket) {
    ...
    onlineUsers[socket.id] = socket.request.session.userId; //for the list of online users

    socket.on("My chat message", msg => {
        db.addMessage(msg, userId).then(msgResult => {
            db.getProfile(userId)
                .then(usrResult => {
                    let merged = { ...msgResult[0], ...usrResult[0] };

                    io.sockets.emit("My chat message", merged);
                })
